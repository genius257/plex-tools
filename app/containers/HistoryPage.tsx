...
export default function HistoryPage() {
  ...
  const rows = plex.db
    .prepare(
      'SELECT * FROM metadata_item_views WHERE account_id = ? ORDER BY viewed_at DESC LIMIT 5'
    )
    .all(userId);
  // console.log(rows);
    ...
    <>
      ...
      <div className={[styles.page, styles.content].join(' ')}>
        {rows.map((row: Record<string, string>) => {
          const guid = row.grandparent_guid || row.guid;
            ...
            <div key={row.id} style={{ marginBottom: '10px' }}>
              ...
              <br />
              {
                plex.db
                  ...
                  .get(row.account_id).name
              }
              <br />
              {
                plex.db
                  .prepare('SELECT * FROM devices WHERE id = ? LIMIT 1')
                  .get(row.device_id).name
              }
              <br />
              <img
                alt="thumbnail"
                width="330"
                src={plex.ConvertURI(row.thumb_url, hash)}
              />
            </div>
          );
        })}
